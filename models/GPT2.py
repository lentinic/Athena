...
class GPTBase(nn.Module):
    def __init__(self, layers:int, dim:int, heads:int, seqlen:int, bias:bool, dropout_p:float):
        ...
        self.emb = nn.Embedding.from_pretrained((torch.rand(256, dim-16) - 0.5).sign())
        self.pos = nn.Embedding.from_pretrained((torch.rand(seqlen, 16) - 0.5).sign())
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None) -> torch.Tensor:
        B, T = x.size()
        idx = torch.arange(0, T, dtype=torch.long, device=x.device).unsqueeze(0)
        rep = [B] + [-1, -1]
        z = torch.cat((self.emb(x), self.pos(idx).expand(*rep)), dim=-1)
        z = self.transformer(z)
        if y is not None:
            return z, F.cross_entropy(z.view(-1, z.size(-1)), y.view(-1), ignore_index=-1)
        return z, None
    
class GPT2(GPTBase, metaclass=Model):
    def __init__(self):
